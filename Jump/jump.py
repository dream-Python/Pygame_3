...
def get_screen_image():
    # 通过ADB命令截取当前手机界面，然后将截图保存在手机的 sd 卡根目录下
...
# 坐标轴单击事件，event 用于获取单击坐标点，coor 用于保存单击的起始坐标点与结束坐标点
def on_click(event):
    if isAuto is False:                    # 判断在没有启动自动跳跃模式时
        if event.xdata is not None and event.ydata is not None:        # 判断获取的坐标点是否为空
            # 获取单击的坐标点并转换为 float 类型
            x = float(event.xdata)
            y = float(event.ydata)
            # 此处判断为了躲避重选与自动按钮的坐标
            if x > 70 and y > 70:
                # 将单击坐标轴上的 x，y 坐标点添加到 coor 列表中
                coor.append((x, y))
                if len(coor) == 1:
                    print('选中起点')
                else:
                ...
                # 如果 coor 列表长度等于 2 时，说明已经获取了单击的其实位置与结束位置的坐标点
                if len(coor) == 2:
...
# 实现跳跃的方法
def jump_to_next(point1, point2):                # 计算跳跃的长度
    x1, y1 = point1                        # 起始点坐标
    x2, y2 = point2                        # 结束点坐标
    # 计算起始点与结束点的距离
    distance = ((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5
    # 执行按压手机屏幕的命令，每个像素点 1.35 毫秒
...
def button_click(event):
    if len(coor) < 2 and ax is not None:
        coor.clear()              # 清空选中坐标点
        ax.lines.clear()          # 清除选中绘制点
        figure.canvas.draw()      # 重画
